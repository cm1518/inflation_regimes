function [ priormean, priorvar ] = get_Minnesota_prior( p, overall_shrinkage, Y, shrink_id)
...
% shrink_id == 1 shrink first lag to identity

[T,N]=size(Y);
K=N*p+1;
if shrink_id
priormean=[zeros(1,N);eye(N);zeros(N*(p-1),N)];
else
priormean=zeros(K,N);
end
